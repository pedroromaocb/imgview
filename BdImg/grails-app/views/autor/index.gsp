		...
		<div id="list-autor" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="nomeAutor" title="${message(code: 'autor.nomeAutor.label', default: 'Nome Autor')}" />
					
						<g:sortableColumn property="emailAutor" title="${message(code: 'autor.emailAutor.label', default: 'Email Autor')}" />
				...
				<g:each in="${autorInstanceList}" status="i" var="autorInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${autorInstance.id}">${fieldValue(bean: autorInstance, field: "nomeAutor")}</g:link></td>
					
						<td>${fieldValue(bean: autorInstance, field: "emailAutor")}</td>
